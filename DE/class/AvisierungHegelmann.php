...
class AvisierungHegelman implements IPreregister{
    ...
    public function insertTableInformations($kennzeichen){
        ...
        $lastRow = $worksheet->getHighestRow();
            for($row=8;$row<=$lastRow;$row++){
                $serviceType = "";
                $ConsigneeReference = "";
                $replaceDash = str_replace("-","",trim($worksheet->getCell("E".$row)->getFormattedValue()));
                ...
                $expl = explode("/",$worksheet->getCell("C".$row)->getFormattedValue());
                $unixTimeStamp = mktime(0,0,0,$expl[0],$expl[1],$expl[2]);
                }
                $transport_for = $worksheet->getCell("I".$row)->getFormattedValue();
                $estUnloadTime = $worksheet->getCell("F".$row)->getFormattedValue();
                if(!empty($knzNummer)){
                    $dataArray[$knzNummer][]=[
